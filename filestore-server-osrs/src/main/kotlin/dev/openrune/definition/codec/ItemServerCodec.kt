...
import dev.openrune.server.impl.item.toServer
...
class ItemServerCodec(
    ...
    override fun ItemServerType.createData() {
        ...
        val infoBox = infoBox!![id]


        id = item.id
        examine = infoBox?.examine?.takeIf { it.isNotEmpty() } ?: ""

        println(infoBox)
        destroy = infoBox?.destroy.takeIf { it.isNullOrEmpty() } ?: ""
        alchable = infoBox?.alchable ?: true
        cost = infoBox?.cost?.takeIf { it != -1 } ?: cost
        exchangeCost = infoBox?.exchangeCost?.takeIf { it != -1 } ?: cost
        ...
        if (item.equipSlot != -1) {
            ...
            equipment = Equipment().apply {
                ...
                requirements = getItemRequirements(params).takeIf { it.isNotEmpty() } ?: infoBox?.itemReq ?: emptyMap()
            ...
            if (item.equipSlot == 3 || item.equipSlot == 5) {
                weapon = Weapon().apply {
                    ...
                    val combatStyle = infoBox?.combatStyle.orEmpty()
                    ...
                    attackRange = params?.getInt(13) ?: infoBox?.attackRange ?: 0
